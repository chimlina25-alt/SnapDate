...
class MemoryService {
  ...
  Stream<List<Memory>> streamMemories(String uid, {int? limit}) {
    return _memories(uid).where('userId', isEqualTo: uid).snapshots().map((
      snap,
    ) {
      final docs = snap.docs.toList();
      docs.sort((a, b) {
        final aTime =
            (a.data()['createdAt'] ?? a.data()['timestamp']) as Timestamp?;
        final bTime =
            (b.data()['createdAt'] ?? b.data()['timestamp']) as Timestamp?;
  ...
  File? _fileFromFileOrXFile(File? file, XFile? xFile) {
  ...
  Future<void> addImageMemory({
    ...
    final fileToUpload = _fileFromFileOrXFile(file, xFile);
    ...
    if (secureUrl == null || secureUrl.isEmpty) {
      throw Exception('Cloudinary image upload failed.');
    }
  ...
  Future<void> deleteMemory({
    ...
    if (storagePath.trim().isNotEmpty &&
        !storagePath.startsWith(RegExp(r'https?://'))) {
