...
class StorageService {
  ...
  Future<StorageUpload> uploadImage({
    ...
    if (kIsWeb) {
      try {
        ...
        final ref = _firebaseStorage.ref().child(
          '$folder/$ownerId/${DateTime.now().microsecondsSinceEpoch}.jpg',
        );
    ...
    // 2. On Mobile, we upload from the picked file, and only store remote URL after success.
    final fileToUse = file ?? (xFile != null ? File(xFile.path) : null);
    if (fileToUse == null || !await fileToUse.exists()) {
      if (xFile == null) {
        throw Exception("No image data provided for upload");
      }
      final uploadBytes = await xFile.readAsBytes();
      final ref = _firebaseStorage.ref().child(
        '$folder/$ownerId/${DateTime.now().microsecondsSinceEpoch}.jpg',
      );
      final metadata = SettableMetadata(
    ...
    await _localStorageService.saveImage(
      file: fileToUse,
      ownerId: ownerId,
      folder: folder,
    );
    ...
    await ref.putFile(fileToUse, metadata);
    final downloadUrl = await ref.getDownloadURL();
    return StorageUpload(url: downloadUrl, path: ref.fullPath);
  }
  ...
  Future<StorageUpload> uploadVideo({
    ...
    if (kIsWeb) {
      try {
        ...
        final ref = _firebaseStorage.ref().child(
          '$folder/$ownerId/${DateTime.now().microsecondsSinceEpoch}$ext',
        );
